...
def test_classifies_when_enabled(monkeypatch):
    ...
    monkeypatch.setattr(seam, "claude_complete", lambda *a, **k: _Reply('{"category": "bug", "reason": "UI broke"}'))
...
def test_falls_back_to_rules_on_bad_output(monkeypatch):
    monkeypatch.setenv("MAAT_TRIAGE_LLM", "1")
    monkeypatch.setattr(seam, "claude_complete", lambda *a, **k: _Reply("not json at all"))
    assert t._llm_triage("x") is None


def test_classify_uses_llm_and_routes_ui_to_autofix(monkeypatch):
    monkeypatch.setenv("MAAT_TRIAGE_LLM", "1")
    monkeypatch.setattr(seam, "claude_complete", lambda *a, **k: _Reply('{"category": "ui", "reason": "layout off"}'))
